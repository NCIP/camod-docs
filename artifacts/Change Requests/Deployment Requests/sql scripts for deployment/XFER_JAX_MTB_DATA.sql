...
BEGIN
	...
	WHERE CARCINOGEN_EXPOSURE_ID >= 50000000
	  AND CARCINOGEN_EXPOSURE_ID < 60000000;
	...
	WHERE ENVIRONMENTAL_FACTOR_ID >= 50000000
	  AND ENVIRONMENTAL_FACTOR_ID < 60000000;
	...
	WHERE HISTOPATHOLOGY_ID >= 50000000
	  AND HISTOPATHOLOGY_ID < 60000000;
	...
	WHERE ABS_CANCER_MODEL_ID >= 50000000
	  AND ABS_CANCER_MODEL_ID < 60000000;
...
BEGIN
	...
	FOR aRec IN getDistinctMTBModels LOOP
		...
		IF ( aRec.TREATMENT_TYPE <> 'None (Spontaneous)' ) THEN
			...
			END IF;
			IF (  aRec.TREATMENT_TYPE = 'Radiation Factor' ) THEN
	...
	FOR aRec IN getMTBHistopathologyData LOOP
	    ...
	    IF LEN(aRec.TUMOR_SYNONYMS) is null THEN
		synonyms := ' ';
	    ELSE
