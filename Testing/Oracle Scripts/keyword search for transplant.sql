...
AND upper(am.model_descriptor) LIKE '%TRANSPLANT%')
...
AND upper(ef.name) LIKE '%TRANSPLANT%'	OR upper(ef.name_Altern_Entry) LIKE '%TRANSPLANT%' )))
...
where am.STATE='Edited-approved'  and av.release_date < sysdate
INTERSECT
SELECT c.abs_cancer_model_id FROM cell_line c WHERE upper(c.name) like '%TRANSPLANT%')
...
WHERE t.agent_id = ag.agent_id AND upper(ag.name) like '%TRANSPLANT%'))
...
AND upper(ph.DESCRIPTION) like '%TRANSPLANT%')
...
IN ( SELECT distinct t.abs_Cancer_Model_Id FROM Transient_Interference t WHERE upper(t.targeted_Region) like '%TRANSPLANT%' ) )
...
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%TRANSPLANT%' AND engineered_gene_type = 'T'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%TRANSPLANT%' AND engineered_gene_type = 'TM'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(clone_designator) LIKE '%TRANSPLANT%' AND engineered_gene_type = 'GS'))
...
WHERE ef.NAME LIKE '%TRANSPLANT%' AND ef.ENVIRONMENTAL_FACTOR_ID = eg2.ENVIRONMENTAL_FACTOR_ID) AND engineered_gene_type = 'IM')
...
where upper(o.NAME) like '%TRANSPLANT%')
...
where hist.DISEASE_ID=d.DISEASE_ID AND hist.ABS_CANCER_MODEL_ID IS NOT null AND upper(d.NAME) like '%TRANSPLANT%' )
