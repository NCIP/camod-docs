...
where am.STATE='Edited-approved'  and av.release_date < sysdate
INTERSECT
select am.abs_cancer_model_id from abs_cancer_model am where am.strain_id in 
(select st.strain_id from strain st where st.species_id in (Select sp.species_id from species sp where upper(sp.scientific_name) like '%MAMMARY%')))
