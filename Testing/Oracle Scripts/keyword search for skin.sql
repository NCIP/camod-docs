...
AND upper(am.model_descriptor) LIKE '%SKIN%')
...
AND upper(ef.name) LIKE '%SKIN%'	OR upper(ef.name_Altern_Entry) LIKE '%SKIN%' )))
...
where am.STATE='Edited-approved'  and av.release_date < sysdate
INTERSECT
SELECT c.abs_cancer_model_id FROM cell_line c WHERE upper(c.name) like '%SKIN%')
...
WHERE t.agent_id = ag.agent_id AND upper(ag.name) like '%SKIN%'))
...
AND upper(ph.DESCRIPTION) like '%SKIN%')
...
IN ( SELECT distinct t.abs_Cancer_Model_Id FROM Transient_Interference t WHERE upper(t.targeted_Region) like '%SKIN%' ) )
...
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%SKIN%' AND engineered_gene_type = 'T'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%SKIN%' AND engineered_gene_type = 'TM'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(clone_designator) LIKE '%SKIN%' AND engineered_gene_type = 'GS'))
...
WHERE ef.NAME LIKE '%SKIN%' AND ef.ENVIRONMENTAL_FACTOR_ID = eg2.ENVIRONMENTAL_FACTOR_ID) AND engineered_gene_type = 'IM')
...
where hist.DISEASE_ID=d.DISEASE_ID AND hist.ABS_CANCER_MODEL_ID IS NOT null AND upper(d.NAME) like '%SKIN%' )
