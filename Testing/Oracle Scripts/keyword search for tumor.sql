...
AND upper(am.model_descriptor) LIKE '%TUMOR%')
...
AND upper(ef.name) LIKE '%TUMOR%'	OR upper(ef.name_Altern_Entry) LIKE '%TUMOR%' )))
...
where am.STATE='Edited-approved'  and av.release_date < sysdate
INTERSECT
SELECT c.abs_cancer_model_id FROM cell_line c WHERE upper(c.name) like '%TUMOR%')
...
WHERE t.agent_id = ag.agent_id AND upper(ag.name) like '%TUMOR%'))
...
AND upper(ph.DESCRIPTION) like '%TUMOR%')
...
IN ( SELECT distinct t.abs_Cancer_Model_Id FROM Transient_Interference t WHERE upper(t.targeted_Region) like '%TUMOR%' ) )
...
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%TUMOR%' AND engineered_gene_type = 'T'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(name) LIKE '%TUMOR%' AND engineered_gene_type = 'TM'))
...
INTERSECT
SELECT eg.ABS_CANCER_MODEL_ID FROM engineered_gene eg WHERE eg.ENGINEERED_GENE_ID IN
(SELECT distinct engineered_gene_id FROM engineered_gene WHERE upper(clone_designator) LIKE '%TUMOR%' AND engineered_gene_type = 'GS'))
...
WHERE ef.NAME LIKE '%TUMOR%' AND ef.ENVIRONMENTAL_FACTOR_ID = eg2.ENVIRONMENTAL_FACTOR_ID) AND engineered_gene_type = 'IM')
...
where hist.DISEASE_ID=d.DISEASE_ID AND hist.ABS_CANCER_MODEL_ID IS NOT null AND upper(d.NAME) like '%TUMOR%' )
