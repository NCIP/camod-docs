...
<html>
		 ...
		 if (treeAction != null && treeAction.equals("highlight")  && highlightId.contains(myId) )
          ...
          { %>
							<td width="<%=30*displayNode.getLevel()%>-3">
								>>>
								<img src="images/transparent.gif" border=0 width="<%=30*displayNode.getLevel()%>-3" height="1" />
							</td>
							<%        } 
          else
          { %>
							<td width="<%=30*displayNode.getLevel()%>">
								<img src="images/transparent.gif" border=0 width="<%=30*displayNode.getLevel()%>" height="1" />
							</td>
							<%        } %>
							<%
          ...
          if (displayWebNode.getIconAction().length() > 0)
          {
            ...
            // this web node has an override icon graphic and action, use that instead of the automated folder and leaf icons %>
							<td width="40" align="right">
								<a onClick="<%=displayWebNode.getIconAction()%>"><img src="skins/<%=skin%>/images/<%=displayWebNode.getIconGraphic()%>" vspace="0" hspace="0" border="0" height="20" width="30" alt="<%=webNodeInfo%>" /></a>
							</td>
							<%        }              
 ...
 %>
							<td width="40" align="right">
								<a href="WebTree.jsp?targetId=<%=java.net.URLEncoder.encode(myId)%>&treeAction=collapse&amp;skin=<%=skin%>"><img src="skins/<%=skin%>/images/folderOpen.gif" vspace="0" hspace="0" border="0" height="20" width="30" alt="<%=webNodeInfo%>" /></a>
								</div>
							</td>
							<%        }
          ...
          { 
            ...
            // no children currently displayed, but has children %>
							<td width="40" align="right">
								<a href="WebTree.jsp?targetId=<%=java.net.URLEncoder.encode(myId)%>&amp;treeAction=expand&amp;skin=<%=skin%>"><img src="skins/<%=skin%>/images/folderClosed.gif" vspace="0" hspace="0" border="0" height="20" width="30" alt="<%=webNodeInfo%>" /></a>
								</div>
							</td>
							<%        }   
          else
          {
            ...
            // no children display, and has not children at all, this is a leaf %>
							<td width="40" align="right">
								<img src="skins/<%=skin%>/images/leaf.gif" vspace="0" hspace="0" border="0" height="20" width="30" alt="<%=webNodeInfo%>" />
							</td>
							<%        }   
